...
A rolling reading log of `arXiv astro-ph.GA` papers — weekly reviews and occasional daily snapshots on high-redshift galaxies, the IGM/CGM, quasar environments, cosmic-web structure, reionization, and related topics. Reports live under year/month folders such as [`2026/06/`](2026/06/); the most recent weekly review is [`2026/06/arXiv_20260612w.md`](2026/06/arXiv_20260612w.md). New reports arrive on a rolling cadence, primarily weekly with occasional daily snapshots; older years accumulate under sibling `YYYY/MM/` folders as the log grows.
...
New reports open with a read-first shortlist and concise scientific findings, followed by thematic paper summaries and abstract links. Source coverage, reading depth, and the drafting model appear in a compact footer; recurring screening instructions live in the agent workflow. Historical reports retain their original format.
...
2. **Ask.** In an LLM coding agent with web access (Claude Code, Codex CLI, or similar) that has this repo as its working directory, drop a short request like:
   ...
   The agent reads the README, follows the pointer to `doc/agent_workflow.md`, picks a sensible date window, scans `arXiv astro-ph.GA`, ranks the hits against your profile, and writes a fresh `YYYY/MM/arXiv_*.md` in the documented format. This can be made automatic by putting it into a routine.
...
Dual-licensed. Workflow, scripts, and templates are MIT-licensed; the dated reports under `YYYY/MM/` are released under Creative Commons Attribution 4.0 International (CC BY 4.0). See [LICENSE](LICENSE) for details.

## Acknowledgments

- Report drafting uses the task's configured model, disclosed in each new report's `Working model:` footer line (older reports use a header).
